...
export function PromptView() {
  ...
  const { mutate: handleGenerate } = useMutation({
    mutationFn: async (content: Content) => {
      if (!isSupabaseConfigured() || !supabase) {
        throw new Error('Backend not configured. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.');
      }
      ...
      generateConversationTitle(conversation.id, content)
        .then(async (title) => {
          if (supabase && title !== 'New Conversation') {
            await supabase
              .from('conversations')
              .update({ title })
    ...
    onError: (error) => {
      ...
      const is404 =
        ...
        (error && typeof error === 'object' && 'code' in error &&
          String((error as { code?: string }).code).startsWith('PGRST'));
      const hint = is404
        ? ' Run: npx supabase db push (then restart the app).'
        : '';
      toast({
        title: 'Error',
        description: (message + hint).trim() || 'Failed to process prompt',
        variant: 'destructive',
      });
    },
  });
  ...
  return (
    ...
    >
      <div
        className={cn(
          'h-full min-h-full bg-grainz-bg-secondary-dark',
      ...
      >
        <main className="w-full px-4 pt-14 sm:pt-12 md:px-8">
          <div className="mx-auto mt-20 flex max-w-3xl flex-col items-center justify-center">
            <h1
              className={cn(
                'mb-8 text-center text-2xl font-medium text-grainz-text-primary md:text-3xl lg:text-4xl',
            ...
            >
              ...
              >
                <TextAreaChat
                  ...
                  placeholder="Start building with Grainz3D..."
                  model={model}
                  setModel={setModel}
              ...
              <div className="relative">
                {isLoading && (
                  <div className="absolute left-0 right-0 top-0">
                    <div className="h-5 w-5 animate-spin rounded-full border-2 border-grainz-blue border-t-transparent" />
                  </div>
                )}
              </div>
            </div>
