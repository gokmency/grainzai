    ...
    <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          variant="ghost"
          className={cn(
            'flex h-8 w-auto items-center gap-1.5 rounded-lg px-3 text-sm transition-all duration-200 hover:border-[#333333] hover:bg-grainz-neutral-800',
            focused
              ? 'text-white hover:text-white'
              : 'text-grainz-text-secondary hover:text-grainz-text-primary',
            isDropdownOpen &&
              (focused
                ? 'bg-grainz-neutral-800 text-white'
                : 'bg-grainz-neutral-800 text-grainz-text-primary'),
            className,
          )}
          disabled={!!disabled}
        >
      ...
      <DropdownMenuContent
        className="flex w-64 flex-col gap-1 rounded-lg bg-grainz-neutral-700 p-1"
        align="end"
      ...
      >
        {models.map((model) => (
          <DropdownMenuItem
            key={model.id}
            className={cn(
              'cursor-pointer rounded-md bg-grainz-neutral-700 px-4 py-3 transition-colors duration-150 focus:bg-grainz-bg-secondary-dark',
              selectedModel === model.id && 'bg-grainz-neutral-800',
            )}
            onClick={(event) => {
            ...
            <div className="flex-1">
              <div className="flex items-center">
                <span
                  className={cn(
                    'font-medium',
                    focused ? 'text-white' : 'text-grainz-text-primary',
                  )}
                >
                  {model.name}
                </span>
