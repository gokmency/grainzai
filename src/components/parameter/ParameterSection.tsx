...
export function ParameterSection() {
  ...
  return (
    <div className="h-full w-full max-w-full border-l border-gray-200/20 bg-grainz-bg-secondary-dark dark:border-gray-800">
      <div className="flex h-14 items-center justify-between border-b border-grainz-neutral-700 bg-gradient-to-r from-grainz-bg-secondary-dark to-grainz-bg-secondary-dark/95 px-6 py-6">
        <div className="flex items-center gap-2">
          <span className="text-lg font-semibold tracking-tight text-grainz-text-primary">
            Parameters
          </span>
        </div>
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                variant="ghost"
                className="h-8 w-8 rounded-full p-0 text-grainz-text-primary transition-colors [@media(hover:hover)]:hover:bg-grainz-neutral-950 [@media(hover:hover)]:hover:text-grainz-neutral-10"
      ...
      <div className="flex h-[calc(100%-3.5rem)] flex-col justify-between overflow-hidden">
        ...
        <div className="flex flex-col gap-4 border-t border-grainz-neutral-700 px-6 py-6">
          <div>
            <ColorPicker />
          </div>
          <div className="flex">
            <Button
              ...
              className="h-12 flex-1 rounded-r-none bg-grainz-neutral-50 text-grainz-neutral-800 hover:bg-grainz-neutral-100 hover:text-grainz-neutral-900"
            >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  className="h-12 w-12 rounded-l-none border-l border-grainz-neutral-300 bg-grainz-neutral-50 p-0 text-grainz-neutral-800 hover:bg-grainz-neutral-100 hover:text-grainz-neutral-900"
                >
              ...
              <DropdownMenuContent
                align="end"
                className="w-64 border-none bg-grainz-neutral-800 shadow-md"
              >
                <DropdownMenuItem
                  onClick={() => setSelectedFormat('stl')}
                  disabled={!blob}
                  className="cursor-pointer text-grainz-text-primary"
                >
                  <span className="text-sm">.STL</span>
                  <span className="ml-3 text-xs text-grainz-text-primary/60">
                    3D Printing
                  </span>
                </DropdownMenuItem>
                <DropdownMenuItem
