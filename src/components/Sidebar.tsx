...
export function Sidebar({ isSidebarOpen }: SidebarProps) {
  ...
  return (
    <div
      className={`${isSidebarOpen ? 'w-64' : 'w-16'} flex h-full flex-shrink-0 flex-col bg-grainz-bg-dark pb-2 transition-all duration-300 ease-in-out`}
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        >
          ...
          >
            <div className="ml-[9px]">
              ...
              >
                <Plus
                  className={`h-5 w-5 ${!isSidebarOpen ? 'text-grainz-neutral-300 hover:text-grainz-text-primary' : ''}`}
                />
                {isSidebarOpen && (
                  <div className="text-sm font-semibold leading-[14px] tracking-[-0.14px] text-grainz-neutral-200">
                    New Creation
                  </div>
                )}
              </Button>
              ...
              <div key={label} className="space-y-1">
                ...
                >
                  <Link to={href}>
                    <Button
                      variant={
                        isSidebarOpen ? 'grainz_dark' : 'grainz_dark_collapsed'
                      }
                ...
                {isSidebarOpen && submenu && (
                  <ul className="ml-7 flex list-none flex-col gap-1 border-l border-grainz-neutral-500 px-2">
                    {submenu.map(
                      (
                        conversation: Omit<
                          Conversation,
                        ...
                        return (
                          ...
                          >
                            <li key={conversation.id}>
                              <span className="line-clamp-1 text-ellipsis text-nowrap rounded-md p-1 text-xs font-medium text-grainz-neutral-400 transition-colors duration-200 ease-in-out [@media(hover:hover)]:hover:bg-grainz-neutral-950 [@media(hover:hover)]:hover:text-grainz-neutral-10">
                                {conversation.title}
                              </span>
                            </li>
                          </Link>
        ...
        >
          ...
          >
            <Link to="https://github.com/Grainz3D/grainz3d" target="_blank">
              <Button
                variant={isSidebarOpen ? 'grainz_dark' : 'grainz_dark_collapsed'}
