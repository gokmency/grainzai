    ...
    <div className="flex flex-col items-center justify-center">
      <div className="relative h-32 w-32">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="h-16 w-16 animate-spin rounded-full border-4 border-grainz-neutral-800 border-t-grainz-blue" />
        </div>
      </div>
      {message && (
        <p
          ref={loadingMessage}
          className="mt-4 text-base text-grainz-text-primary transition-opacity duration-200"
        >
          {changingMessage}
          <span>.</span>
          <span
