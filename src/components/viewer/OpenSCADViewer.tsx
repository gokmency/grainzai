...
export function OpenSCADViewer() {
  ...
  return (
    <div className="h-full w-full bg-grainz-neutral-700/50 shadow-lg backdrop-blur-sm transition-all duration-300 ease-in-out">
      <div className="h-full w-full">
        ...
        {isCompiling && (
          <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-grainz-neutral-700/30 backdrop-blur-sm">
            <div className="flex flex-col items-center gap-3">
              <Loader2 className="h-6 w-6 animate-spin text-grainz-blue" />
              <p className="text-xs font-medium text-grainz-text-primary/70">
                Compiling...
              </p>
            </div>
          </div>
    ...
    <div className="flex h-full flex-col items-center justify-center gap-4 p-6">
      <div className="flex flex-col items-center gap-3">
        <div className="relative">
          <div className="absolute inset-0 animate-ping rounded-full bg-grainz-blue/20" />
          <CircleAlert className="h-8 w-8 text-grainz-blue" />
        </div>
        <div className="text-center">
          <p className="text-sm font-medium text-grainz-blue">
      ...
      {fixError && error && error.name === 'OpenSCADError' && (
        <Button
          ...
          className={cn(
            ...
            'hover:border-grainz-blue/70 hover:bg-grainz-blue/50 hover:text-white',
            'hover:shadow-[0_0_25px_rgba(167,139,250,0.4)]',
            'focus:outline-none focus:ring-2 focus:ring-grainz-blue/30',
          )}
          onClick={() => {
        ...
        >
          <div className="absolute inset-0 rounded-lg bg-gradient-to-br from-grainz-blue/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
