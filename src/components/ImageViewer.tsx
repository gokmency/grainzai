...
export function ImageViewer({
  ...
  const { data: imageUrl, isLoading: isImageLoading } = useQuery({
    ...
    enabled: !!image && isSupabaseConfigured(),
    queryFn: async () => {
  ...
  if (isImageLoading) {
    return (
      <div
        className={cn(
          'flex aspect-square w-full items-center justify-center rounded-lg text-grainz-text-primary',
          className,
        )}
      >
  ...
  if (!imageUrl) {
    return (
      <div
        className={cn(
          'flex aspect-square h-full w-full items-center justify-center rounded-lg text-grainz-text-primary',
          className,
        )}
      >
  ...
  return (
    <div className="flex h-full w-full items-center justify-center">
      ...
      >
        ...
        />
        {!loaded && (
          <div className="absolute inset-0 flex items-center justify-center bg-grainz-neutral-800/50 text-grainz-text-primary">
            <Loader2 className="h-10 w-10 animate-spin" />
          </div>
        )}
        {clickable && (
          <>
            ...
            <div
              className={`absolute left-2 top-2 z-10 rounded-full p-1 ${isSelected ? 'bg-grainz-blue' : 'bg-black'} cursor-pointer transition-transform duration-200 hover:scale-110 ${isSelected ? 'opacity-100' : hoverable ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'}`}
              onClick={(e) => {
                e.stopPropagation();
                selectItem({
                  id: image,
