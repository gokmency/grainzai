...
function TextAreaChat({
  onSubmit,
  placeholder = 'What can Grainz3D help you build today?',
  disabled = false,
  model,
  setModel,
  conversation,
  ...
  const handleImageRemoved = async (image: MessageItem) => {
    if (!image.isUploading) {
      ...
      if (image.source === 'upload' && supabase) {
        try {
  ...
  const generatePrompt = async () => {
    ...
    if (!supabase) {
      toast({
        ...
        description: 'Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to use this feature.',
        variant: 'destructive',
      });
      return;
    }
    ...
    >
                  ...
                  <AnimatePresence>
                    ...
                    {images.map((image) => (
                      ...
                      >
                        ...
                        {image.source === 'mesh-render' && (
                          <div className="absolute bottom-0 left-0 rounded-br-md rounded-tl-md bg-grainz-blue/80 p-0.5">
                        ...
                        <button
                          ...
                          className={cn(
                            'absolute right-[-0.50rem] top-[-0.50rem] rounded-full border border-grainz-neutral-500 bg-grainz-neutral-500 text-white transition-colors duration-200 hover:border-grainz-neutral-700 hover:bg-grainz-neutral-700',
      ...
      >
        <div className="flex select-none items-center justify-between p-2">
          <Avatar className="h-8 w-8 border border-grainz-neutral-700 bg-grainz-neutral-950">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 flex-shrink-0 rounded-full hover:bg-grainz-neutral-800"
                  onClick={(e) => {
                    e.stopPropagation();
                    generatePrompt();
                  }}
                ...
                >
                  {isGeneratingPrompt ? (
                    <Loader2 className="h-4 w-4 animate-spin text-grainz-blue" />
                  ) : (
        ...
        <div className="flex items-center justify-between border-t border-[#2a2a2a] p-3">
          <div className="flex items-center gap-3">
            ...
            >
              <Button
                variant="outline"
                className="flex h-8 w-8 items-center gap-2 rounded-lg border border-[#2a2a2a] bg-grainz-background-2 p-0 text-sm text-grainz-text-secondary hover:bg-grainz-bg-secondary-dark"
                onClick={(e) => {
                  e.stopPropagation();
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                'flex h-8 w-8 transform items-center justify-center rounded-lg bg-grainz-neutral-700 p-1 text-white transition-all duration-300 hover:scale-105 hover:bg-grainz-blue/90 disabled:opacity-50 disabled:hover:scale-100 disabled:hover:bg-grainz-blue',
