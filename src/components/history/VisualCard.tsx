        ...
        <div className="relative aspect-square w-full overflow-hidden bg-gradient-to-br from-grainz-background-1 to-grainz-background-2">
          {!isVisible ? (
            <div className="flex h-full w-full items-center justify-center">
              <Box className="text-grainz-neutral-600 h-16 w-16 opacity-30" />
            </div>
          ) : isCompiling ? (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex flex-col items-center gap-2">
                <Loader2 className="h-8 w-8 animate-spin text-grainz-blue" />
                <span className="text-xs text-grainz-neutral-400">
                  Compiling...
                </span>
              </div>
            </div>
            ...
            <div className="flex h-full w-full items-center justify-center">
              <Box className="text-grainz-neutral-600 h-16 w-16 opacity-50" />
            </div>
          ) : (
            <ThreePreview geometry={geometry} />
          )}

          <div className="absolute inset-0 bg-gradient-to-t from-grainz-background-2/90 via-transparent to-transparent opacity-0 transition-opacity duration-200 group-hover:opacity-100" />
        </div>

        <div className="p-4">
          <h3 className="mb-2 line-clamp-2 text-base font-medium text-grainz-neutral-50">
            {conversation.title}
          </h3>
          <div className="flex items-center gap-3 text-xs text-grainz-neutral-400">
        ...
        <AlertDialog>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="h-8 w-8 rounded-full bg-grainz-background-1/80 p-0 backdrop-blur-sm transition-colors duration-200 hover:bg-grainz-neutral-950"
                onClick={(e) => e.stopPropagation()}
              >
                <MoreVertical className="h-4 w-4 text-grainz-neutral-50" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="bg-[#191A1A]">
              <DropdownMenuItem
                ...
                className="text-grainz-neutral-50 hover:cursor-pointer hover:bg-grainz-neutral-950 focus:bg-grainz-neutral-950"
              >
                <Pencil className="mr-2 h-4 w-4" />
                Rename
              </DropdownMenuItem>
              <AlertDialogTrigger asChild onClick={(e) => e.stopPropagation()}>
                <DropdownMenuItem className="text-grainz-neutral-50 hover:cursor-pointer hover:bg-grainz-neutral-950 hover:text-red-500 focus:bg-grainz-neutral-950 focus:text-red-500">
          ...
          <AlertDialogContent className="border-[2px] border-grainz-neutral-700 bg-grainz-background-1">
            <AlertDialogHeader>
              <AlertDialogTitle className="text-grainz-neutral-100">
                Delete Creation
              </AlertDialogTitle>
              <AlertDialogDescription>
