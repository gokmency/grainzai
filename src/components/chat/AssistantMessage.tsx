...
export function AssistantMessage({
  ...
  return (
    <div className="flex justify-start">
      ...
      <div className="w-[80%] rounded-lg bg-grainz-neutral-800">
        <div className="flex flex-col gap-3 p-3 text-sm text-grainz-text-primary">
                  ...
                  <div className="flex w-full flex-col gap-2">
                    {message.content.toolCalls.map((toolCall) => (
                      <div
                        ...
                        className="flex h-10 w-full items-center justify-between overflow-hidden rounded-md bg-grainz-neutral-950 px-3 hover:bg-grainz-neutral-900"
                      >
          ...
          {(message.siblings.length > 1 ||
            ...
            <div className="flex items-center gap-1">
              {!isLastMessage && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <History className="h-3 w-3 p-0 text-grainz-neutral-100" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    <span>Restore</span>
                ...
                <div className="flex items-center">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <RefreshCw className="h-3 w-3 p-0 text-grainz-neutral-100" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      <span>Retry</span>
              ...
              {message.siblings.length > 1 && (
                <div className="flex h-6 items-center gap-0.5 rounded-lg border border-grainz-neutral-700 bg-grainz-bg-secondary-dark">
                  <Button
                  ...
                  >
                    <ChevronLeft className="h-3 w-3 p-0 text-grainz-neutral-100" />
                  </Button>
                  <span className="text-xs tracking-widest text-grainz-neutral-100">
                  ...
                  >
                    <ChevronRight className="h-3 w-3 p-0 text-grainz-neutral-100" />
                  </Button>
                </div>
              )}
            </div>
...
function ObjectButton({
  ...
  return (
    <Button
      ...
      className={cn(
        'group relative bg-black p-2 hover:bg-grainz-bg-dark',
    ...
    >
      <div className="flex w-full items-center justify-between border-gray-200/20 pr-16 dark:border-gray-700">
        <div className="flex min-w-0 items-center space-x-2">
          <Box className="h-4 w-4 shrink-0 text-grainz-text-primary" />
          <span className="truncate font-medium text-grainz-text-primary">
            {title}
          </span>
        </div>
        <span
          className={cn(
            'absolute right-2 flex h-6 items-center overflow-hidden rounded-md border border-grainz-neutral-700 bg-grainz-bg-secondary-dark px-1 text-xs transition-all duration-100 ease-in-out hover:bg-black',
            isHovered
              ? 'w-14 text-grainz-text-primary'
              : `w-${6 + (currentVersion.toString().length - 1)} text-grainz-neutral-300`,
          )}
        >
          {isHovered ? (
            <div className="flex items-center gap-1">
  ...
  if (availableModels.length === 0) {
    return (
      <Button
        ...
        disabled={true}
        className={cn(
          'h-6 w-fit gap-1 rounded-lg px-2 text-xs text-grainz-text-primary opacity-50',
          className,
        )}
      >
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          disabled={disabled}
          className={cn(
            'h-6 w-fit gap-1 rounded-lg px-2 text-xs text-grainz-text-primary',
            isOpen && 'bg-grainz-neutral-800',
            className,
          )}
        >
          <span>{selectedModelConfig.name}</span>
      ...
      <DropdownMenuContent
        className="w-48 rounded-lg border border-grainz-neutral-700 bg-grainz-neutral-800 p-1"
        align="start"
      >
        {availableModels.map((model) => (
          <DropdownMenuItem
            key={model.id}
            className={cn(
              'cursor-pointer rounded-md bg-grainz-neutral-800 px-2 py-1.5 text-xs text-grainz-text-primary hover:bg-grainz-neutral-700 focus:bg-grainz-bg-secondary-dark',
            )}
            onClick={() => {
              if (onRetry) {
                onRetry(model.id);
